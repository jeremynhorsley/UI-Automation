...
# Check if communications will be overriden
$xml = [xml](get-content $SetpointPath)
$xml.Load($SetpointPath)
$xmlAI = ($xml.AppData.Interface.Group.Array | Where-Object {$_.name -match "MasterConfiguration"})
if($xmlAI)
{
    $msg =  "Loading setpoints would override communication setup"
    $success = $false
    $pfarr = $fail
}
if(!$pfarr)
